...
export default function SellerBlog() {
  ...
  const submit = async (finalize = false) => {
    try {
      ...
      if ((res.data?.success ?? false) || res.data?._id) {
        await fetchMyPosts();
        setForm({ title: "", slug: "", cover: "", category: "General", tags: "", excerpt: "", content: "", seoTitle: "", seoDescription: "" });
  ...
  const statusColor = (s: SellerPost["status"]) =>
    s === "published" ? "bg-green-100 text-green-800" : s === "pending_review" ? "bg-yellow-100 text-yellow-800" : s === "draft" ? "bg-gray-100 text-gray-800" : "bg-red-100 text-red-800";
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea className="min-h-[180px]" value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} />
          </div>
          ...
          <div className="flex gap-2">
            <Button disabled={loading} onClick={() => submit(false)}>Save Draft</Button>
            <Button disabled={loading} className="bg-[#C70000] hover:bg-[#A60000] text-white" onClick={() => submit(true)}>Submit for Review</Button>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {posts.length === 0 ? (
            ...
            <div className="space-y-3">
              {posts.map((p) => (
                <div key={p._id} className="border rounded-lg p-3 flex items-center justify-between">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a className="text-sm text-blue-600 hover:underline" href={`/blog/${p.slug}`} target="_blank" rel="noreferrer">Preview</a>
                  </div>
                </div>
              ))}
            </div>
